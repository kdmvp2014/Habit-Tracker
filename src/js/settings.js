...
// ── PROFIL: Geburtsdatum, Größe, Gewicht ──────────────────
function validateProfile(birthdate, heightCm, weightKg){
...
function commitProfile(birthdate, heightCm, weightKg){
  S.profile = { birthdate, heightCm: parseFloat(heightCm), weightKg: parseFloat(weightKg) };
  save();
}
...
export function saveProfileSettings(){
  ...
  const err = validateProfile(birthdate, heightCm, weightKg);
...
export function openProfileSetupModal(){
  ...
  document.getElementById('psm-birthdate').value = '';
  document.getElementById('psm-height').value = '';
  document.getElementById('psm-weight').value = '';
